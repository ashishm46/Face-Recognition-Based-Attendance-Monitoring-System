...
def submit_live_attendance(stu_id, student_class,date):
    attendance_class={
        "cse" : "INSERT INTO cse(student_id,attendance_date) VALUES(?, ?);",
        "ece" : "INSERT INTO ece(student_id,attendance_date) VALUES(?, ?);",
    }
...
def fetchAttendance(attendanceClass, attendanceDate):
    student_class={
        'ece' : "SELECT DISTINCT(d.student_id),d.fullname,d.class,ac.attendance_date FROM ece ac,student_data d WHERE ac.student_id=d.student_id AND attendance_date = ?;",
        'cse' : "SELECT DISTINCT(d.student_id),d.fullname,d.class,ac.attendance_date FROM cse ac,student_data d WHERE ac.student_id=d.student_id AND attendance_date = ?;",
    }
    conn = create_connection()
    cursor = conn.cursor()
    val = [attendanceDate]
...
def fetch_graph_data(graphClass):
    student_class = {
        'ece' : "SELECT DISTINCT(attendance_date) FROM ece ORDER BY attendance_date ASC LIMIT 06 ",
        'cse' : "SELECT DISTINCT(attendance_date) FROM cse ORDER BY attendance_date ASC LIMIT 06 ",
    }
    attendance_class = {
        'ece' : "SELECT COUNT(DISTINCT(student_id)) FROM ece WHERE attendance_date = ? ;",
        'cse' : "SELECT COUNT(DISTINCT(student_id)) FROM cse WHERE attendance_date = ? ;",
    }
